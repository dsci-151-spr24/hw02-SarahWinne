...
***The median is less affected by potential outliers, so more accurately reflects the center of the data***
...
***I chose a binwidth of 5000 because it gave a clearer image of the general distribution. It also just has one peak bin instead of two***
...
***The min and max are good for figuring out spread. The median gives the center and seeing the q1 and q3, having one closer to the median could show the right skewed distribution. Having the mean and median together could help see the distribution too, with the mean being larger than the median, but that could also be seen from the quartiles.***
...
college_recent_grads %>%
  ...
  arrange(med = desc(med))
```

***The major category Engineering has the highest typical median income.***
...
***Having an interdisciplinary major is the least popular in this sample.***
...
***There tend to be a higher proportion of women in non-stem fields, which tend to have lower median incomes.***

## Further exploration

10. Ask a question of interest to you, and answer it using summary statistic(s) and/or visualization(s).

Is there a connection between unemployment and whether its a stem major?

```{r}
ggplot(data = college_recent_grads, mapping = aes(x = unemployment_rate)) +
  geom_histogram(binwidth = .015) +
  facet_wrap(~major_type)
```
As far as I can see, there is not much difference, or it's more difficult to see due to there being much more non stem majors. 
